...
export function initServer(serverOptions: Partial<ServerOptions>): {
  ...
  // Disable WPPConnect default banner
  defaultLogger.level = 'warn'; // This will suppress the banner
  
  // After server setup, reset logger level to configured value
  setTimeout(() => {
  ...
  http.listen(PORT, () => {
    ...
    console.log('\x1b[34m%s\x1b[0m', banner); // Add blue color to banner
    logger.info(`Server is running on port: ${PORT}`);
    logger.info(
      `\x1b[31m Visit ${serverOptions.host}:${PORT}/api-docs for Swagger docs`
    );
    logger.info(`FusionHub Server version: ${version}`);
